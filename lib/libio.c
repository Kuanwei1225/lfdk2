...
void PrintIOScreen(int fd)
{
    ...
    if (enter_mem) {
        ...
        if (ibuf == KEY_UP) {
            ...
            if ((0xffffffff - ioaddr) >= LFDK_MASSBUF_SIZE) {

                ioaddr += LFDK_MASSBUF_SIZE;
            } else {
            ...
            if (ioaddr >= LFDK_MASSBUF_SIZE) {

                ioaddr -= LFDK_MASSBUF_SIZE;
            } else {

                ioaddr = (0xffffffff - LFDK_MASSBUF_SIZE + 1);
            }
    ...
    for (i = 0; i < LFDK_BYTE_PER_LINE; i++) {
        ...
        for (j = 0; j < LFDK_BYTE_PER_LINE; j++) {
            ...
            tmp = ((unsigned char)lfdk_io_data.mass_buf[(i * LFDK_BYTE_PER_LINE) + j]);
            if ((tmp >= '!') && (tmp <= '~')) {
    ...
    for (i = 0; i < LFDK_BYTE_PER_LINE; i++) {
        ...
        for (j = 0; j < LFDK_BYTE_PER_LINE; j++) {
            ...
            } else if (((unsigned char)lfdk_io_data.mass_buf[(i * LFDK_BYTE_PER_LINE) + j])) {
            ...
            if (y == j && x == i) {
                ...
                if (input) {
                    ...
                    wprintw(IOScreen.value, "%2.2X", (unsigned char)lfdk_io_data.mass_buf[(i * LFDK_BYTE_PER_LINE) + j]);
                }
            } else {

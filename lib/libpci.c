        ...
        for (dev = 0; dev <= 0x1f; dev++) {
            ...
            for (fun = 0; fun <= 0x07; fun++) {
                ...
                if ((lfdd_pci_data.buf & 0xffff) != 0xffff) {
                    ...
                    // Get Texts
                    //
                    GetVendorAndDeviceTexts(lfdd_pci_list[last_index].venid, lfdd_pci_list[last_index].devid, lfdd_pci_list[last_index].ventxt, lfdd_pci_list[last_index].devtxt);
...
void PrintPCIScreen(int fd)
{
    ...
    //
    PrintFixedWin(PCIScreen, info, 1, 47, 22, 0, WHITE_BLUE, "Type: PCI    Bus %2.2X    Device %2.2X    Function %2.2X", lfdd_pci_list[curr_index].bus, lfdd_pci_list[curr_index].dev, lfdd_pci_list[curr_index].fun);
